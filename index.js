...
function submitUsernames(user1, user2) {
  ...
  Promise.all(promises).then(scores => {
    ...
    scores.forEach(score => {
      localStorage.setItem(`duelLingoScore_${score.user}`, score.value);
    });

    document.querySelector('.outputArea').innerHTML = JSON.stringify(localStorage);
...
function getCurrentScore(userToFind) {
  ...
  return fetch(url, headers)
    ...
    .then(data => {
      ...
      const user = users.find(user => user.username.toLowerCase() === userToFind);
